...
ghitm(struct monst * mtmp, struct obj * gold)
{
    ...
    if (!likes_gold(mtmp->data) && !mtmp->isshk && !mtmp->ispriest &&
        ...
        if (canseemon(mtmp)) {
            pline("The %s harmlessly %s %s.", xname(gold), otense(gold, "hit"),
                  mon_nam(mtmp));
            msg_given = TRUE;
        }
    } else {
        ...
        if (mtmp->isshk) {
            ...
            if (robbed) {
                ...
                if (mtmp->mpeaceful) {
                    ...
                    pline("You have %ld %s in credit.", ESHK(mtmp)->credit,
...
container_impact_dmg(struct obj *obj)
{
    ...
    if (costly && loss) {
        if (!insider) {
            ...
            pline("You owe %s %ld %s for objects destroyed.", mon_nam(shkp),
                  loss, currency(loss));
        }
    }
}
...
impact_drop(struct obj *missile, xchar x, xchar y, xchar dlev)
{
    ...
    if (costly && shkp && price) {
        ...
        if (ESHK(shkp)->debit > debit) {
            ...
            pline("You owe %s %ld %s for goods lost.", Monnam(shkp), amt,
                  currency(amt));
        }
    }
