...
/*119*/ 15, 15, 15, 15, 14, 14, 14, 13, 13, 12, 11, 10, 9, 8, 6, 3,
    /* 135 */ 16
        /* should be MAX_RADIUS+1; used to terminate range loops -dlc */
};
...
const char circle_start[] = {
    /* */ 0,
    /* circles of radius zero are not used */
/* 1*/ 0,
/* 2*/ 2,
/* 3*/ 5,
/* 4*/ 9,
 ...
 */
int
does_block(struct level *lev, int x, int y)
{
    struct monst *mon;
             ...
             char *rmin, char *rmax)
{
    ...
    if (rnum >= 0) {
        for (zy = level->rooms[rnum].ly - 1; zy <= level->rooms[rnum].hy + 1;
             zy++) {
 ...
 */
static int
q1_path(int srow, int scol, int y2, int x2)
{
    int dx, dy;
 ...
 */
static int
q4_path(int srow, int scol, int y2, int x2)
{
    int dx, dy;
    int k, err, x, y, dxs, dys;
 ...
 */
static int
q2_path(int srow, int scol, int y2, int x2)
{
    int dx, dy;
    int k, err, x, y, dxs, dys;
 ...
 */
static int
q3_path(int srow, int scol, int y2, int x2)
{
    int dx, dy;
    int k, err, x, y, dxs, dys;
 ...
 */
static void
right_side(int row,     /* current row */
    ...
    while (left <= right_mark) {
        ...
        if (!is_clear(row, left)) {
            ...
            if (right_edge > right_mark) {
                ...
                right_edge =
                    is_clear(row - step,
                             right_mark) ? right_mark + 1 : right_mark;
            }
            if (vis_func) {
 ...
 */
static void
left_side(int row, int left_mark, int right, const char *limits)
{
    ...
    while (right >= left_mark) {
        ...
        if (!is_clear(row, right)) {
            ...
            if (left_edge < left_mark) {
                ...
                left_edge =
                    is_clear(row - step, left_mark) ? left_mark - 1 : left_mark;
            }
            if (vis_func) {
 ...
 */
void
do_clear_area(int scol, int srow, int range, void (*func) (int, int, void *),
              void *arg)
{
